...
public class GameBoardDisplay {
	...
	private String displayColumnHeadersFirstLine() {
		StringBuilder lineColumnHeaders = new StringBuilder(EMPTY_LINE_BLOCK);

		for (int column = 1; column <= Constants.COLUMNS; column++) {
			lineColumnHeaders.append(BOTTOM_LINE_BLOCK);
		}
		lineColumnHeaders.append(LINE_BREAK);
		return lineColumnHeaders.toString();
	}
